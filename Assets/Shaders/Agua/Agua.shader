Shader "pepe/Agua"
{
    Properties
    {
        [Header(Espesor)]
        [Space(5)]
        _DepthDensity ("Profundidad", Range(0.0, 1.0)) = 0.5
        _DistanceDensity ("Distancia", Range(0.0, 1.0)) = 0.1
        
        [Header(Olas)]
        [Space(5)]
        [NoScaleOffset]
        _WaveNormalMap ("Normal Map", 2D) = "bump"{}
        _WaveNormalScale ("Escala", float) = 10.0
        _WaveNormalSpeed ("Velocidad", float) = 1.0

        [Header(Color Profundidad)]
        [Space(5)]
        [HDR]
        _ShallowColor ("Cerca", Color) = (0.44, 0.95, 0.36, 1.0)
        [HDR]
        _DeepColor ("Medio", Color) =  (0.0, 0.05, 0.19, 1.0)
        [HDR]
        _FarColor ("Lejos", Color) = (0.04, 0.27, 0.75, 1.0)
        
        [Header(Reflejos)]
        [Space(5)]
        _ReflectionContribution ("Contribucion", Range(0.0, 1.0)) = 1.0

        [Header(Dispersion Subsuelo)]
        [Space(5)]
        [HDR]
        ...
        [Header(Espuma)]
        [Space(5)]
        _FoamContribution ("Contribucion", Range(0.0, 1.0)) = 1.0
        [NoScaleOffset]
        _FoamTexture ("Textura", 2D) = "black"{}
        _FoamScale ("Escala", float) = 1.0
        _FoamSpeed ("Velocidad", float) = 1.0
        _FoamNoiseScale ("Ruido Contribucion", Range(0.0, 1.0)) = 0.5

        [Header(Sol Specular)]
        [Space(5)]
        [HDR]
        _SunSpecularColor ("Color", Color) = (1, 1, 1, 1)
        _SunSpecularExponent ("Exponente", float) = 1000

        [Header(Brillo)]
        [Space(5)]
        [NoScaleOffset]
        _SparklesNormalMap ("Normal Map", 2D) = "bump"{}
        _SparkleScale ("Escala", float) = 10
        _SparkleSpeed ("Velocidad", float) = 0.75
        [HDR]
        _SparkleColor ("Color", Color) = (1, 1, 1, 1)
        _SparkleExponent ("Exponente", float) = 10000

        [Header(Espuma Cresta)]
        [Space(5)]
        [HDR]
        _EdgeFoamColor ("Color", Color) = (1, 1, 1, 1)
        _EdgeFoamDepth ("Escala", float) = 10.0

        [Header(Shadow Mapping)]
        [Space(5)]
        [Toggle(SHADOWS)]
        _FancyShadows("Activado", int) = 0
        _MaxShadowDistance("Maxima Distancia Muestra", float) = 50.0
        _ShadowColor ("Color", Color) = (0.5, 0.5, 0.5, 1.0)

        [Header(Olas Vertex 1)]
        [Space(5)]
        _Wave1Direction ("Direccion", Range(0, 1)) = 0
        _Wave1Amplitude ("Amplitudad", float) = 1
        _Wave1Wavelength ("Longitud Ola", float) = 1
        _Wave1Speed ("Velocidad", float) = 1

        [Header(Olas Vertex 2)]
        [Space(5)]
        _Wave2Direction ("Direccion", Range(0, 1)) = 0
        _Wave2Amplitude ("Amplitud", float) = 1
        _Wave2Wavelength ("Longitud Ola", float) = 1
        _Wave2Speed ("Velocidad", float) = 1
    }
    SubShader
    {
        LOD 100
